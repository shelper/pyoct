...
def calc_dc(data, axis=-1, method='mean'):
    if method == 'mean':
        dc = data.mean(0)
    elif method == 'median':
...
if __name__ == '__main__':
    ...
    for i in range(1, 12, 1):
        data = read_binary_data('T:\ZYtemp\\2012-05-30 benchtop sdoct\\signal roll off\\OCTImage' + "%.2d" % i + '.raw',
                                data_dim=1024, input_format='uint16')
